      ...
      >
        ...
        {!hideChallenge && headAuthor?.username !== user?.username && (
          <button aria-label="challenge" onClick={(e) => { 
            ...
            <span className={`${challengeCount > 0 ? 'text-[9px]' : 'text-[6px]'} max-w-[30px] font-semibold text-white leading-none text-center whitespace-nowrap`}>{countLabel(challengeCount, 'Challenge')}</span>
          </button>
        )}
        <button aria-label="comments" onClick={(e) => { 
          ...
          <span className={`${commentCount > 0 ? 'text-[9px]' : 'text-[6px]'} max-w-[30px] font-semibold text-white leading-none text-center whitespace-nowrap`}>{countLabel(commentCount, 'Comment')}</span>
        </button>
        ...
        {/* Disco de música (estilo TikTok) — ahora con aspecto de VINILO REAL:
            ...
            pausa) — reemplaza el disco fijo anterior. */}
