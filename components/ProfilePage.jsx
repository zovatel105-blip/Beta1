...
export const GridItem = ({ post, onOpen }) => {
  ...
  return (
    ...
    >
      ...
      {/* Contador - píldora abajo-izquierda: SOLO reproducciones/vistas
          (petición del usuario: antes mostraba votos arriba + vistas debajo,
          ahora solo vistas). Perfil propio Y ajeno. */}
      {views > 0 && (
...
export default function ProfilePage({ open, onClose, onOpenUpload, onChallenge, onRequireAuth, onRequireLogin, onOpenProfile, onPostViewerChange, username = null }) {
  ...
  // También se oculta mientras la Cartera (WalletSheet) o la hoja de Propina
  // (TipSheet) están abiertas (petición del usuario: "no debe aparecer la
  // barra de navegación inferior cuando se abre este modal") — ambas son
  // overlays de pantalla completa (fixed inset-0), la barra no debe quedar
  // visible por debajo/detrás de ellas.
  useEffect(() => {
    onPostViewerChange?.(!!openPost || walletOpen || tipOpen)
    return () => { onPostViewerChange?.(false) }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [openPost, walletOpen, tipOpen])
  ...
  return (
    <div ref={scrollRef} onScroll={handleScroll} className={`fixed inset-0 ${(menuOpen || editOpen || followList) ? 'z-[90]' : 'z-40'} bg-[#0a0a0b] overflow-y-auto overscroll-contain`}>
      ...
      <div ref={barRef} className="sticky top-0 z-30 bg-[#0a0a0b]"
           style={{ paddingTop: 'max(env(safe-area-inset-top), 6px)' }}>
        <div className="grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-4 max-[390px]:gap-2 px-2 sm:px-4 h-11 max-w-md mx-auto w-full">
          ...
          <div className="relative z-10 flex items-center gap-2 max-[390px]:gap-1 shrink-0 justify-self-end">
            {isOwn ? (
              <>
                <button
              ...
              <>
                ...
                <button
                  ...
                  disabled={followBusy}
                  style={{ opacity: revealP, pointerEvents: revealP > 0.5 ? 'auto' : 'none' }}
                  className={`h-7 shrink-0 min-w-[92px] px-5 rounded-full font-semibold text-[12px] tracking-tight active:scale-95 transition-transform disabled:opacity-60 flex items-center justify-center max-[390px]:min-w-0 max-[390px]:px-2.5 max-[390px]:text-[11px] max-[390px]:tracking-normal max-[340px]:px-1.5 ${
