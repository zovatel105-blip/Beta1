...
// week: impreso esta semana → penalización fuerte de score (×0.30).
async function getSeenSets(viewerKey) {
  const out = { hour: new Set(), week: new Set() }
...
export async function rankFeed(candidates, { viewerKey, context = {}, limit = 8, cursor = 0 } = {}) {
  ...
  const scored = list.map((c) => {
    ...
    //    éxitos = votos + completions ponderadas; fracasos = impresiones sin acción.
    //    Item nuevo (0 imp) → Beta(1,1) ancha → exploración natural.
    const posE = tot + 0.6 * completes
    ...
    const seenPen = seen.week.has(c.id) ? 0.30 : 1
    ...
    const score = base * wv.mult * neg * seenPen * votedPen
    ...
    return {
      ...
      dbg: {
        ...
        seen: seenPen < 1, voted: votedPen < 1, imp, votes: tot,
      },
    }
  })
  ...
  // Re-ranking de diversidad (MMR). La exploración ya la aporta Thompson
  // Sampling, así que epsilon queda casi a cero. La seed depende de la salt
  // de sesión (antes era fija por viewer → orden congelado para siempre).
  const ranked = mmrReRank(scored, { epsilon: 0.03, seed: hashSeed(salt) })
