...
import { putBuffer } from './gridfs'

const EBSYNTH_BIN = nodePath.join(process.cwd(), '.bin', 'ebsynth')
const WORK_DIR = nodePath.join(process.cwd(), '.tmp_ai_video')
// Ya NO se usa como almacenamiento persistente (ver lib/gridfs.js) — el
// vídeo final de cada job se escribe primero aquí (ffmpeg necesita una ruta
// de archivo real, no puede escribir directo a GridFS) y luego se sube a
// GridFS; el fichero de esta carpeta se borra justo después.
const UPLOAD_DIR = WORK_DIR
...
async function runCompositeMode({ jobId, jobs, apiKey, videoPath, prompt, work, moving }) {
  ...
  // Sube el resultado final a GridFS (compartido entre réplicas) y borra el
  // archivo temporal de scratch — ffmpeg necesitaba una ruta real para
  // escribir el vídeo, pero el almacenamiento persistente ya no es disco.
  const outBuffer = await fs.readFile(outVideoPath)
  await putBuffer(outFilename, outBuffer, 'video/mp4')
  await fs.rm(outVideoPath, { force: true }).catch(() => {})
