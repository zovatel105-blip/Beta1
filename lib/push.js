...
export async function sendPush({ userId, type, fromUser, postId = null, commentId = null }) {
  try {
    ...
    // Limpieza de tokens inválidos/desinstalados (FCM los reporta como error
    // 'messaging/registration-token-not-registered' o similar).
    const invalidTokens = result.responses
      .map((r, i) => (!r.success && /registration-token/.test(r.error?.code || '') ? devices[i].token : null))
      .filter(Boolean)
