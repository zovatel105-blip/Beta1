...
function normalizeRuleConfig(ruleConfig) {
  ...
  return {
    ...
    // Phase 2/3 additive, JSON-serializable alternatives to the code-only
    // `combine` (INPUT_VALIDATION) and `comparator` (COMPARISON) extension
    // points -- see evaluateCombineSpec()/evaluateComparatorSpec() in
    // lib/challengeRuleEngine.js. Stored-but-inert unless the relevant rule
    // actually reads it; an event with neither field is unaffected.
    ...
    // `comparatorSpec` above are the JSON-serializable, data-authored
    // alternatives for authors who can't attach a live function.
  }
}
