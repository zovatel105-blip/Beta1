...
function normalizeRuleConfig(ruleConfig) {
  ...
  return {
    ...
    // Additive, JSON-serializable alternative to the code-only `validate`
    // extension point (INPUT_VALIDATION only) -- see evaluateValidationSpec()
    // in lib/challengeRuleEngine.js. Stored-but-inert unless a step wires the
    // interpreter in; an event with only `validValues` is unaffected.
    validationSpec: (cfg.validationSpec && typeof cfg.validationSpec === 'object') ? cfg.validationSpec : undefined,
    // Phase 2/3/4 additive, JSON-serializable alternatives to the code-only
    // `combine` (INPUT_VALIDATION), `comparator` (COMPARISON), and
    // `condition` (SURVIVAL) extension points -- see
    // evaluateCombineSpec()/evaluateComparatorSpec()/evaluateConditionSpec()
    // in lib/challengeRuleEngine.js. Stored-but-inert unless the relevant
    // rule actually reads it; an event with neither field is unaffected.
    combineSpec: (cfg.combineSpec && typeof cfg.combineSpec === 'object') ? cfg.combineSpec : undefined,
    comparatorSpec: (cfg.comparatorSpec && typeof cfg.comparatorSpec === 'object') ? cfg.comparatorSpec : undefined,
    conditionSpec: (cfg.conditionSpec && typeof cfg.conditionSpec === 'object') ? cfg.conditionSpec : undefined,
    ...
    // this phase exactly. 'vote_tally' remains a supported, additive
    // opt-in for any OTHER PREDICTION event a client explicitly requests
    // it for — but Team vs Team specifically is FORCED to 'reveal'
    // server-side in createUniversalChallenge below (see that function's
    // comment) whenever participantStructure is TEAM_VS_TEAM and rule is
    // PREDICTION: viewer votes there are predictions about a real result
    // the creator confirms (`correctOptionId`), never a live tally that
    // decides the winner, so a client can never turn 'vote_tally' ON for
    // that combination either.
    outcomeMode: cfg.outcomeMode === 'vote_tally' ? 'vote_tally' : 'reveal',
    // `condition`/`validate`/`combine`/`comparator` (the live-function
    // forms) are intentionally NOT persisted (functions aren't
    // JSON-serializable) — pluggability for those remains a code-level
    // extension point (pass event objects with live functions directly to
    // the Rule Engine in-process, e.g. from a test), not something
    // authored through the API. `validationSpec`/`combineSpec`/
    // `comparatorSpec`/`conditionSpec` above are the JSON-serializable,
    // data-authored alternatives for authors who can't attach a live
    // function.
  }
}
...
export async function createUniversalChallenge(postId, authorId, { videoId, participants, events, teams, entities, groups } = {}) {
...
export async function getUniversalChallengeByPostId(postId) {
  ...
  return ensureEntitiesAndGroups(strip(doc))
}
