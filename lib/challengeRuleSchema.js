...
export const RULE_SCHEMA = {
  ...
  [RULES.PREDICTION]: {
    ...
    own: {
      ...
      // Default 'option' reproduces EVERY existing PREDICTION event
      // unchanged (discrete "[ALEX] [DAVID] [TIE]" flow, Team vs Team,
      // Guess the Actor/Action, etc. — none of them ever set this key, so
      // they all coerce to 'option' exactly as before this field existed).
      // 'numeric' is the new mode: viewers submit a raw number instead of
      // picking an option/participant (see prediction() in
      // lib/challengeRuleEngine.js and castUniversalChallengeInput in
      // lib/universalChallengeStore.js).
      predictionType: { type: 'enum', values: ['option', 'numeric'], default: 'option' },
      // The real-world numeric answer, set by the creator (same
      // authoring-time convention `correctOptionId` already uses — see
      // its own comment above; there is no separate "reveal later" API
      // for EITHER field today). Only consulted when predictionType is
      // 'numeric'; `null` means "not revealed yet".
      correctValue: { type: 'number|null', default: null },
    },
  },
...
function coerceField(def, raw) {
  switch (def.type) {
    ...
    // STEP 3 (Advanced Mode gap-fill, numeric prediction) — additive type,
    // used only by PREDICTION's new `correctValue`. Any non-finite input
    // (missing, empty string, non-numeric text) falls back to the field's
    // own default (`null`) rather than persisting `NaN`.
    case 'number|null': {
      const n = Number(raw)
