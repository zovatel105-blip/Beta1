...
export const RULE_SCHEMA = {
  ...
  [RULES.INPUT_VALIDATION]: {
    ...
    own: {
      ...
      // `validate`/`combine` remain code-only extension points (functions
      // aren't JSON-serializable) — not represented here as data fields.
    },
  },
  [RULES.SURVIVAL]: {
    universal: { ...MISSING_SUBMISSION_FIELDS },
    own: {
      ...
      // `condition` remains a code-only extension point.
    },
  },
  [RULES.COMPARISON]: {
    universal: { ...TIE_BREAK_FIELDS, ...TEAM_AGGREGATION_FIELDS },
    own: {
      // `comparator` remains a code-only extension point.
    },
  },
}
