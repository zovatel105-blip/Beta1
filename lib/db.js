...
// Emails con rol admin (además del primer usuario registrado). Configurable por
// la variable de entorno ADMIN_EMAILS (lista separada por comas). Se incluye un
// valor por defecto para el administrador principal.
...
export async function createUser({ username, email, password, avatarUrl = null }) {
  ...
  // Rol admin: el PRIMER usuario registrado, o cualquier email de la lista admin.
  const totalUsers = await users.countDocuments({})
  const role = (totalUsers === 0 || isAdminEmail(email)) ? 'admin' : 'user'
