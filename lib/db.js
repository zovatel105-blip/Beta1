...
// leaderboard interno de CADA tema, que sí compite por votos+IA).
export async function listCommunityLuxuryThemes(limit = 24) {
  const col = await getCollection(COLLECTIONS.LUXURY_THEMES)
  return await col.find({ source: 'user' }).sort({ createdAt: -1 }).limit(limit).toArray()
}
