...
export const COLLECTIONS = {
  ...
  MARKETING_PLAYBOOK_LOG: 'marketingPlaybookLog',
}
...
/**
 * MARKETING PLAYBOOK (panel de admin) — registro diario de publicación
 * (posted, idea de contenido usada, hashtags, sonido, notas) para llevar la
 * racha de días consecutivos publicando, siguiendo la cadencia del playbook
 * (ver lib/marketingPlaybook.js). Una entrada por fecha ('YYYY-MM-DD').
 */

// Últimas N entradas del registro (por defecto 60 días), más recientes primero.
export async function getMarketingPlaybookLogs(limit = 60) {
  const col = await getCollection(COLLECTIONS.MARKETING_PLAYBOOK_LOG)
  const docs = await col.find({}).sort({ date: -1 }).limit(limit).toArray()
  return docs.map((d) => { const { _id, ...rest } = d; return rest })
}

// Crea o actualiza la entrada del día (upsert por fecha).
export async function upsertMarketingPlaybookLog({ date, posted, contentIdea, hashtags, sound, notes, updatedBy }) {
  const col = await getCollection(COLLECTIONS.MARKETING_PLAYBOOK_LOG)
  const now = new Date()
  const update = {
    date,
    posted: !!posted,
    contentIdea: typeof contentIdea === 'string' ? contentIdea.slice(0, 200) : '',
  ...
  await col.updateOne(
    { date },
    { $set: update, $setOnInsert: { id: generateId(), createdAt: now } },
    { upsert: true }
  )
...
// Racha de días consecutivos con `posted: true`, contando hacia atrás desde
// HOY (UTC). Si hoy aún no se registró, la racha se cuenta desde ayer.
