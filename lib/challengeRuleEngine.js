...
// FASTEST — lowest recorded time wins.
function fastest(event, inputs, participantStates) {
  return measurementVerdict(event, inputs, participantStates, { pickBest: (vals) => Math.min(...vals), field: 'timeMs' })
}
...
//     SEQUENTIAL_MATCHUP's `insufficientVotesFallback` behavior. This is
//     what powers Team vs Team's community-vote path (see
//     lib/universalChallengeParticipantStructures.js STRUCTURE_RULE /
//     lib/universalChallengeStore.js createUniversalChallenge, which
//     forces this mode + derives `options` from the two teams server-
//     side whenever a TEAM_VS_TEAM event picks the PREDICTION rule) —
//     the two teams ARE the options, so the winning option IS the
//     winning team, guaranteed to be exactly one, never a creator-preset
//     value. Still zero new entries in RULE_IMPLS — PREDICTION is reused
//     as-is, just with an additive config-gated branch.
...
function inputValidation(event, inputs, participantStates) {
  ...
  // Validation step: default is a configurable valid-values allow-list
  // (event.ruleConfig.validValues); pluggable via event.ruleConfig.validate.
...
function survival(event, inputs, participantStates) {
  ...
  if (event.ruleConfig?.conditionMode === 'per_participant_pass_fail') {
    ...
    const condition = typeof event.ruleConfig?.condition === 'function' ? event.ruleConfig.condition : () => false
...
// rule out of the box, not a placeholder.
...
export function resolveEvent(event, inputs, participantStates) {
  ...
  if (!optionsOnly && (!Array.isArray(event.participantIds) || event.participantIds.length === 0)) {
