...
// "Luxury Battle" — retos ABIERTOS ("Open"/antes "Single", ver
// getOpenChallengeFeedItems en route.js) etiquetados con un tema de lujo
// (challenge.luxuryThemeId). A diferencia de los retos 1v1, estos NUNCA
// llegan a `posts` (viven siempre en `challenges`) — por eso necesitan su
// propia consulta para el leaderboard (ver GET /api/luxury-battles/
// leaderboard) y su propio guardado de puntaje de IA de UN SOLO lado (ver
// scoreLuxuryOpenEntry, route.js).
export async function getOpenLuxuryChallengesByThemeId(themeId) {
  if (!themeId) return []
  const col = await getCollection(CHALLENGES)
  const docs = await col.find({ open: true, luxuryThemeId: themeId }).sort({ _seq: -1 }).toArray()
  return docs.map(strip)
}

export async function updateChallengeLuxuryScore(challengeId, { scoreA, verdictA }) {
  const col = await getCollection(CHALLENGES)
  await col.updateOne(
    { id: challengeId },
    { $set: { 'luxuryBattle.scoreA': scoreA ?? null, 'luxuryBattle.verdictA': verdictA || '' } }
  )
}
