 ...
 *
 *   - TEAM_VS_TEAM — CUSTOMER'S FINAL, UNAMBIGUOUS DECISION (overrides the
 *     earlier "creator chooses aggregated measurement vs community vote"
 *     design this module originally shipped): ALWAYS community-vote-only.
 *     Locked to exactly one rule, PREDICTION — same as e.g. GUESS_THE_ACTOR
 *     — see STRUCTURE_RULE below; there is no measurement-rule path left at
 ...
 *     below), tallied by the ordinary PREDICTION path with
 *     `ruleConfig.outcomeMode: 'vote_tally'` (also forced server-side);
 *     `computeTeamVoteStandings` below reads the winner straight off the
 *     already-resolved `result.winners`, never re-deriving it. Not a new
 *     rule: PREDICTION is reused exactly as-is. `computeTeamStandings`
 *     (the original measurement-based aggregator) is kept below as a
 *     generic reader but is no longer reachable through this structure.
...
export const STRUCTURE_RULE = {
  ...
  // TEAM_VS_TEAM — CUSTOMER'S FINAL, UNAMBIGUOUS DECISION: Team vs Team is
  // ALWAYS community-vote-only, with NO "aggregated measurement" option at
  // all (supersedes the earlier phase's "let the creator choose between
  // aggregated measurement and community vote" design). Locked to exactly
  // ONE rule — PREDICTION — the same way SEQUENTIAL_TOURNAMENT/
  // GUESS_THE_ACTOR/etc. are locked to one rule each, so
  // `structureAllowedRules`/`structureRuleOf` below can never surface
  ...
  // createUniversalChallenge, which additionally forces
  // `ruleConfig.outcomeMode: 'vote_tally'` and derives `options` from the
  // two teams server-side — never trusting a client-supplied value for
  // either). Still not a new rule: PREDICTION is reused exactly as-is.
...
export const STRUCTURE_META = {
  ...
  [PARTICIPANT_STRUCTURES.TEAM_VS_TEAM]: {
    ...
    desc: 'Two teams face off — viewers vote live for the team they think should win.',
...
// TEAM_VS_TEAM — COMMUNITY-VOTE variant (additive, Phase 3). Companion to
// `computeTeamStandings` above, for a TEAM_VS_TEAM event authored with
// `rule: PREDICTION` / `ruleConfig.outcomeMode: 'vote_tally'` instead of a
// measurement rule: here the two teams ARE the vote options (`event.
// options` = [{id: teamAId, label}, {id: teamBId, label}], forced
// server-side — see universalChallengeStore.js createUniversalChallenge),
// so "aggregate" is each team's live vote COUNT, not a summed/averaged
// measurement, and the winning team is read straight off the Rule
// Engine's own already-resolved `event.result.winners` (a single teamId,
// guaranteed by `resolveTie()` — see challengeRuleEngine.js `prediction()`)
// rather than re-derived by comparing aggregates here — this function
// never decides a winner on its own, purely presents one already decided.
// `memberValues` carries participantIds through (values always null —
// no per-participant number exists for a vote) purely so the SAME
// `TeamVsTeamMomentView` (UniversalChallengeMomentViews.jsx) can keep
// rendering avatar stacks with zero component-level branching between the
// two variants.
