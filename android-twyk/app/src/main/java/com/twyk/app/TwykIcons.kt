...
// NATIVA se vea IGUAL que la web (lucide-react). El color lo aplica el `tint`
// del Icon (ColorFilter), por eso los paths van en negro.
//   · vote     -> "papeleta marcada" (tick en V). Sólido = votado.
//   · Share    -> flecha de envío (TikTok).
//   · Swords   -> espadas cruzadas (Retar / Batallas).
//   · home     -> casa (relleno = activo).
//   · Inbox    -> bandeja.
//   · User     -> persona.
//   · Comment  -> bocadillo redondo (lucide MessageCircle).
//   · bookmark -> marcador (relleno = guardado).
// ─────────────────────────────────────────────────────────────────────────────
object TwykIcons {

    // Vote sólido (V relleno) -> votado.
    ...
    // Vote outline (doble contorno) -> sin votar.
    ...
    private const val HOME =
        "M3 9 L12 2 L21 9 V20 A2 2 0 0 1 19 22 H5 A2 2 0 0 1 3 20 Z M9 22 L9 12 L15 12 L15 22"
    private const val INBOX =
        "M22 12 H16 L14 15 H10 L8 12 H2 M5.45 5.11 L2 12 V18 A2 2 0 0 0 4 20 H20 A2 2 0 0 0 22 18 V12 L18.55 5.11 A2 2 0 0 0 16.76 4 H7.24 A2 2 0 0 0 5.45 5.11 Z"
    private const val USER =
        "M19 21 V19 A4 4 0 0 0 15 15 H9 A4 4 0 0 0 5 19 V21 M16 7 A4 4 0 1 1 8 7 A4 4 0 1 1 16 7 Z"
    private const val COMMENT =
        "M7.9 20 A9 9 0 1 0 4 16.1 L2 22 Z"
    private const val BOOKMARK =
        "M19 21 L12 17 L5 21 V5 A2 2 0 0 1 7 3 H17 A2 2 0 0 1 19 5 Z"

    private var voteSolid: ImageVector? = null
    private var voteOutline: ImageVector? = null
    private var shareC: ImageVector? = null
    private var swordsC: ImageVector? = null
    private var homeO: ImageVector? = null
    private var homeF: ImageVector? = null
    private var inboxC: ImageVector? = null
    private var userC: ImageVector? = null
    private var commentC: ImageVector? = null
    private var bookmarkO: ImageVector? = null
    private var bookmarkF: ImageVector? = null

    fun vote(filled: Boolean): ImageVector =
        if (filled) voteSolid ?: buildVote(VOTE_SOLID, PathFillType.NonZero).also { voteSolid = it }
        else voteOutline ?: buildVote(VOTE_OUTLINE, PathFillType.EvenOdd).also { voteOutline = it }

    val Share: ImageVector get() = shareC ?: stroke("TwykShare", SHARE, 1.5f).also { shareC = it }
    val Swords: ImageVector get() = swordsC ?: stroke("TwykSwords", SWORDS, 1.7f).also { swordsC = it }
    val Inbox: ImageVector get() = inboxC ?: stroke("TwykInbox", INBOX, 2f).also { inboxC = it }
    val User: ImageVector get() = userC ?: stroke("TwykUser", USER, 2f).also { userC = it }
    val Comment: ImageVector get() = commentC ?: stroke("TwykComment", COMMENT, 1.9f).also { commentC = it }

    fun home(filled: Boolean): ImageVector =
        if (filled) homeF ?: fill("TwykHomeF", HOME).also { homeF = it }
        else homeO ?: stroke("TwykHome", HOME, 2f).also { homeO = it }

    fun bookmark(filled: Boolean): ImageVector =
        if (filled) bookmarkF ?: fill("TwykBmF", BOOKMARK).also { bookmarkF = it }
        else bookmarkO ?: stroke("TwykBm", BOOKMARK, 1.9f).also { bookmarkO = it }

    // ── builders ─────────────────────────────────────────────────────────────
    private fun stroke(name: String, path: String, width: Float): ImageVector =
    ...
    // El path del vote vive en coordenadas grandes con transform SVG
    // "translate(0,600) scale(0.1,-0.1)" -> lo replicamos con un grupo.
    private fun buildVote(path: String, fillType: PathFillType): ImageVector =
        ImageVector.Builder("TwykVote", 24.dp, 24.dp, 600f, 600f).apply {
            group(scaleX = 0.1f, scaleY = -0.1f, translationX = 0f, translationY = 600f) {
                addPath(pathData = addPathNodes(path), pathFillType = fillType, fill = SolidColor(Color.Black))
            }
        }.build()
}
