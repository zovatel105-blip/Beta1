...
class VideoPool {
  ...
  _swapSource(player, mp4Url, posterUrl) {
    // Poster (primer fotograma) para mostrar algo al instante mientras carga.
    if (posterUrl && player.video.poster !== posterUrl) {
      try { player.video.poster = posterUrl } catch (e) { /* ignore */ }
    }
    if (player.src === mp4Url) return
    // Pause first to free decoder
    try { player.video.pause() } catch (e) { /* ignore */ }
    player.mp4Source.src = mp4Url
    // Solo añadimos la fuente webm para vídeos integrados (/videos/*.webm existen).
    // Los uploads pueden no tener webm transcodificado todavía -> evitamos 404s
    // que en dev disparan un render de página completo y ralentizan el feed.
    if (typeof mp4Url === 'string' && mp4Url.startsWith('/videos/')) {
      player.webmSource.src = mp4Url.replace(/\.mp4$/, '.webm')
    } else {
      player.webmSource.removeAttribute('src')
    }
    player.src = mp4Url
    // load() forces the element to (re)read its <source> children
    try { player.video.load() } catch (e) { /* ignore */ }
  }

  /** Get a player ready to display `mp4Url` for `slideId`. */
  acquire(slideId, mp4Url, posterUrl) {
    ...
    this._swapSource(player, mp4Url, posterUrl)
  ...
  release(slideId) {
    ...
    if (!p) return
    try { p.video.pause() } catch (e) { /* ignore */ }
    p.state = 'IDLE'
 ...
 *  Pool size = 8: un dueto consume 2 reproductores, así que con 8 cabe el slide
 *  activo (2) + anterior (2) + siguiente (2) aunque los tres sean duetos, con
 *  margen para el scroll-back rápido. Evita el "robo" de reproductores que
 *  provocaba recargas y tirones en las publicaciones 1vs1. */
export function getVideoPool() {
  if (typeof window === 'undefined') return null
  if (!_pool) {
    _pool = new VideoPool(8)
