...
const VOTES = 'votes'
const TIP_CHALLENGES = 'tipChallenges'
...
// Elimina una publicación propia tipo "open challenge" (BUG #244618: el feed
// las expone con id `open_<challenge_id>` pero el documento vive en
// `challenges`; deletePostById solo mira `posts` -> 404 siempre). Propiedad
// verificada contra `from.id` (mismo criterio que setChallengeAllowChallenge).
// Devuelve { ok, reason?, mediaUrls } — mediaUrls para limpieza en GridFS.
export async function deleteOpenChallenge(feedId, ownerId) {
  const realId = String(feedId || '').replace(/^open_/, '')
  const col = await getCollection(CHALLENGES)
  const doc = await col.findOne({ id: realId })
  if (!doc) return { ok: false, reason: 'not_found' }
  const owner = doc.from?.id || null
  if (!ownerId || !owner || owner !== ownerId) {
    return { ok: false, reason: 'forbidden' }
  }
  await col.deleteOne({ id: realId })
  // Limpieza best-effort de datos asociados (no bloquea el borrado principal).
  try {
    const comments = await getCollection('comments')
    await comments.deleteMany({ postId: feedId })
    const saves = await getCollection('saves')
    await saves.deleteMany({ postId: feedId })
    const views = await getCollection('singleViews')
    await views.deleteMany({ postId: feedId })
    const votes = await getCollection('singleVotes')
    await votes.deleteMany({ postId: feedId })
  } catch { /* noop */ }
  const mediaUrls = [doc.challengerVideoUrl, doc.challengerImageUrl, doc.challengerPosterUrl,
    doc.targetVideoUrl, doc.targetImageUrl, doc.targetPosterUrl].filter(Boolean)
...
// ────────────────────────────────────────────────────────────────────────────
// TIP CHALLENGES — "propina para proponer un reto" (perfil ajeno): el botón
// de Propina (créditos, ver TipSheet.jsx) ahora, en vez de solo transferir
// créditos sueltos, crea una PROPUESTA de reto con los créditos en ESCROW
// (ya restados del emisor, ver POST /api/tip-challenges/lib/db.js
// deductWalletCredits). El destinatario puede Aceptar (sube su vídeo/foto
// completando el reto) o Rechazar (reembolso inmediato); si acepta y sube
// su respuesta, el EMISOR original debe darle el "visto bueno" (Aprobar)
// para que los créditos se liberen de verdad al destinatario — o
// Rechazar la entrega, lo que reembolsa al emisor. Ver
// NotificationsInbox.jsx para las tarjetas accionables de cada estado.
//
// Estados: pending -> submitted -> approved | declined
//                  -> rejected (rechazo inicial, antes de enviar nada)
// ────────────────────────────────────────────────────────────────────────────

export async function getAllTipChallenges() {
  const col = await getCollection(TIP_CHALLENGES)
  const docs = await col.find({}).sort({ _seq: -1 }).toArray()
  return docs.map(strip)
}

export async function getTipChallengeById(id) {
  const col = await getCollection(TIP_CHALLENGES)
  const doc = await col.findOne({ id })
  return strip(doc)
}

export async function insertTipChallenge(doc) {
  const col = await getCollection(TIP_CHALLENGES)
  await col.insertOne({ ...doc, _seq: nextSeq() })
  return doc
}

// Actualiza SOLO si el estado actual coincide con `expectedStatus` (guarda
// ATÓMICA contra doble-clic/doble-envío: dos peticiones simultáneas para el
// mismo id nunca pueden ambas tener éxito). Devuelve el documento actualizado
// o null si no existía o ya no estaba en el estado esperado.
export async function updateTipChallengeStatus(id, expectedStatus, fields) {
  const col = await getCollection(TIP_CHALLENGES)
  const res = await col.findOneAndUpdate(
